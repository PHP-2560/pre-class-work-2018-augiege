...
game_chain <- function(n, start = 1000, bet = 100, winprob = .48, prob_inc = .01){
  output <- rep(NA, n)
  output[1] <- start
  ...
  for(i in 2:n){
    ...
    # If we already have 100% chance of winning, we automatically fill out the rest of the simulation with wins and return the final amount of money
    else if(winprob1 == 1){
      #print(winprob1)
      for(j in (i+1):n){
    ...
    # For every loss, we set the win probability back to the original 
    else {
      winprob1 <- winprob
      #print(winprob1)
    }
...
expected_value <- sum(replicate(100, game_chain(n=100000)))/100
expected_value
```
